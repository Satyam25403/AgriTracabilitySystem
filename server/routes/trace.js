...
// GET /api/trace/:batchId  - PUBLIC route, no auth required
// Returns complete journey of a batch - used by QR code scan
router.get("/:batchId", async (req, res) => {
  try {
    ...
    const processingLogs = await ProcessingLog.find({ batchId: batch._id })
      .sort({ timestamp: 1 });

    const inventory = await Inventory.findOne({ batchId: batch._id });

    const shipments = await Shipment.find({ batchId: batch._id })
      .sort({ createdAt: -1 });
    ...
    const timeline = [
      {
        stage: "sourced",
        label: "Harvested at Farm",
        location: batch.farmLocation,
        timestamp: batch.createdAt,
        details: `${batch.quantity} ${batch.unit} of ${batch.commodityType} harvested by ${batch.farmerName}`,
        completed: true,
      },
